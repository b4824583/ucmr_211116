...
class MeshPredictor(object):
    def __init__(self, opts):
        ...
        # print(self.model.right_partner_idx)
        ...
        if not opts.textureUnwrapUV:
            ...
            faces_uv_fx3x2_np = self.faces_uv.detach().cpu().numpy()

            self.uv_sampler = mesh.compute_uvsampler_unwrapUV(
                faces_uv_fx3x2_np,
                self.faces.cpu().numpy(),
                tex_size=opts.tex_size,
                shift_uv=opts.texture_uvshift)
    ...
    def forward(self):
        ...
        #----------------------------------------edited by parker
        #-----------------------------------no use  I want delete
        Whether_write_file=False
        if(Whether_write_file):
            delta_v_txt=open("vertices_output/delta_vertice.txt","w")
            for i in range(len(delta_v[0])):
              x=delta_v[0][i][0]
              y=delta_v[0][i][1]
              z=delta_v[0][i][2]

              delta_v_txt.write("{:.8f}".format(x.item())+" "+"{:.8f}".format(y.item())+" "+"{:.8f}".format(z.item())+"\n")
            delta_v_txt.close()
            cams_txt=open("predict_output/cams.txt","w")
            for i in range(cams_bx7.shape[1]):
                cams_txt.write(str(cams_bx7[0][i].item())+" ")
            cams_txt.close()
        #---------------------------------------------
        ...
        if opts.pred_texture:
            ...
            # print(textures)
            # print("max texture size:"+str(torch.max(textures)))
        ...
        # mask_temp=np.zeros((self.rend_mask.shape[1],self.rend_mask.shape[2]))
        # for i in range(self.rend_mask.shape[1]):
        #     for j in range(self.rend_mask.shape[2]):
        #         mask_temp[i][j]=self.rend_mask[0][i][j].item()
        # imsave('predict_output/bird_mask.png', mask_temp)
        # imshow(mask_temp)
        # plt.show()
        #--------------------------------------------------
        if opts.pred_texture:
