...
class NeuralRenderer_pytorch(torch.nn.Module):
    ...
    def project_points_perspective(self, verts, cams, depth=False):
        ...
        if depth:
            ...
            return verts[:, :, :2]
    def proj_fn_verts(self,vertices,cams):
        """
        edited by parker
        Args:
            vertices:
        ...
        """
        offset_z=0
        vertices = self.proj_fn(vertices, cams, offset_z=offset_z)
        vertices[:, :, 1] *= -1
        return vertices
    ...
    def forward(self, vertices, faces, cams, textures=None):
        ...
        if textures is None:
            ...
            # print("masks")
            # print(masks.shape)
            # mask_img=masks[0].detach().cpu().numpy()
            # plt.imshow(mask_img)
            # plt.show()
            # exit()
            return masks
            ...
            # print("masks")
            # print(imgs.shape)
            # img_plt=imgs.detach().cpu().numpy()[0].transpose((1, 2, 0))
            # plt.imshow(img_plt)
            # plt.show()
            return imgs
